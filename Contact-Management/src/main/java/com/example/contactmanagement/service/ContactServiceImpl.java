...
public class ContactServiceImpl implements ContactService{
	...
	public List<ContactResponse> searchContactByLastName(String lastName) {
		...
		List<Contact> contacts = contactRepository.findByLastNameContainingIgnoreCase(lastName);
	...
	public ContactResponse updateContact(Long id, ContactRequest request) {
		...
		if(contact.isPresent()) {	
			Contact presentContact = contact.get();
			presentContact.setFirstName(request.getFirstName());
			presentContact.setLastName(request.getLastName());
			presentContact.setEmail(request.getEmail());
			presentContact.setPhoneNumber(request.getPhoneNumber());
			Contact updatedContact = contactRepository.save(presentContact);
